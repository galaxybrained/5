...

{
    apt update
    apt upgrade -y
    apt install -y cloudflared openssh wget git tmux apache2 curl proot proot-distro expect python python-pip
    ...
    latest_version=$(./pull_code_server.sh | awk '/Latest version:/ {print $3}')
    old_version="4.12.0"
